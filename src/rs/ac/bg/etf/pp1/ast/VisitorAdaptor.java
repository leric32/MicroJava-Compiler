// generated with ast extension for cup
// version 0.8
// 4/0/2023 0:9:16


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(ListOfCharDec ListOfCharDec) { }
    public void visit(Mulop Mulop) { }
    public void visit(MethodDecl MethodDecl) { }
    public void visit(ConstructorDecl ConstructorDecl) { }
    public void visit(Relop Relop) { }
    public void visit(ListOfClassVarDec ListOfClassVarDec) { }
    public void visit(VarDeclBrackets VarDeclBrackets) { }
    public void visit(DesignatorOrEpsilon DesignatorOrEpsilon) { }
    public void visit(FunctionCall FunctionCall) { }
    public void visit(StatementList StatementList) { }
    public void visit(ClassName ClassName) { }
    public void visit(ClassVarDecl ClassVarDecl) { }
    public void visit(ListOfFactors ListOfFactors) { }
    public void visit(Addop Addop) { }
    public void visit(PrintEmpty PrintEmpty) { }
    public void visit(ListOfVarDec ListOfVarDec) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(ForEachLoop ForEachLoop) { }
    public void visit(VarDeclNotLast VarDeclNotLast) { }
    public void visit(Designator Designator) { }
    public void visit(ConstAssignment ConstAssignment) { }
    public void visit(ClassBody ClassBody) { }
    public void visit(FormParsList FormParsList) { }
    public void visit(Condition Condition) { }
    public void visit(IfCond IfCond) { }
    public void visit(ConstructorDeclName ConstructorDeclName) { }
    public void visit(ClassVar ClassVar) { }
    public void visit(WhileLoop WhileLoop) { }
    public void visit(ActualParamList ActualParamList) { }
    public void visit(VarDeclLast VarDeclLast) { }
    public void visit(ConstructorDeclList ConstructorDeclList) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(Expr Expr) { }
    public void visit(ActPars ActPars) { }
    public void visit(MethodTypeName MethodTypeName) { }
    public void visit(DesignatorList DesignatorList) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(OneFormalParam OneFormalParam) { }
    public void visit(FuncCallFactorDes FuncCallFactorDes) { }
    public void visit(Statement Statement) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(ConstructorVarDecl ConstructorVarDecl) { }
    public void visit(ClassDecl ClassDecl) { }
    public void visit(ConstDecl ConstDecl) { }
    public void visit(ClassExtended ClassExtended) { }
    public void visit(CondFact CondFact) { }
    public void visit(MethodVarDeclList MethodVarDeclList) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(FormPars FormPars) { }
    public void visit(ClassMethodDecl ClassMethodDecl) { }
    public void visit(ListOfClassVar ListOfClassVar) { }
    public void visit(ModuoOperator ModuoOperator) { visit(); }
    public void visit(DivideOperator DivideOperator) { visit(); }
    public void visit(MultiplyOperator MultiplyOperator) { visit(); }
    public void visit(MinusOperator MinusOperator) { visit(); }
    public void visit(PlusOperator PlusOperator) { visit(); }
    public void visit(LessEqualOperator LessEqualOperator) { visit(); }
    public void visit(LessOperator LessOperator) { visit(); }
    public void visit(GreaterEqualOperator GreaterEqualOperator) { visit(); }
    public void visit(GreaterOperator GreaterOperator) { visit(); }
    public void visit(NotEqualOperator NotEqualOperator) { visit(); }
    public void visit(EqualOperator EqualOperator) { visit(); }
    public void visit(FuncCallFactorDesignator FuncCallFactorDesignator) { visit(); }
    public void visit(NewClassWithActParsOperatorFactor NewClassWithActParsOperatorFactor) { visit(); }
    public void visit(NewArrayOperatorFactor NewArrayOperatorFactor) { visit(); }
    public void visit(FuncCallFactor FuncCallFactor) { visit(); }
    public void visit(ExprFactor ExprFactor) { visit(); }
    public void visit(BooleanConstFactor BooleanConstFactor) { visit(); }
    public void visit(CharConstFactor CharConstFactor) { visit(); }
    public void visit(NumberConstFactor NumberConstFactor) { visit(); }
    public void visit(DesignatorFactor DesignatorFactor) { visit(); }
    public void visit(SingleFactor SingleFactor) { visit(); }
    public void visit(MultipleFactors MultipleFactors) { visit(); }
    public void visit(Term Term) { visit(); }
    public void visit(TermNegativeExpr TermNegativeExpr) { visit(); }
    public void visit(TermPositiveExpr TermPositiveExpr) { visit(); }
    public void visit(AddExpr AddExpr) { visit(); }
    public void visit(SimpleDesignator SimpleDesignator) { visit(); }
    public void visit(ArrayDesignator ArrayDesignator) { visit(); }
    public void visit(ClassFieldDesignator ClassFieldDesignator) { visit(); }
    public void visit(Epsilon Epsilon) { visit(); }
    public void visit(DesignArrAssignment DesignArrAssignment) { visit(); }
    public void visit(SingleOrEndOfListDesignator SingleOrEndOfListDesignator) { visit(); }
    public void visit(ListOfDesignator ListOfDesignator) { visit(); }
    public void visit(FunctionCallName FunctionCallName) { visit(); }
    public void visit(DesignatorArrayAssignment DesignatorArrayAssignment) { visit(); }
    public void visit(DesignatorDec DesignatorDec) { visit(); }
    public void visit(DesignatorInc DesignatorInc) { visit(); }
    public void visit(DesignatorFuncCall DesignatorFuncCall) { visit(); }
    public void visit(ErrorDesignatorAssignment ErrorDesignatorAssignment) { visit(); }
    public void visit(DesignatorAssignment DesignatorAssignment) { visit(); }
    public void visit(SingleCondition SingleCondition) { visit(); }
    public void visit(RelopCondition RelopCondition) { visit(); }
    public void visit(ConditionsWithoutAnd ConditionsWithoutAnd) { visit(); }
    public void visit(ConditionsWithAnd ConditionsWithAnd) { visit(); }
    public void visit(ConditionsWithoutOr ConditionsWithoutOr) { visit(); }
    public void visit(ConditionsWithOr ConditionsWithOr) { visit(); }
    public void visit(ActualParam ActualParam) { visit(); }
    public void visit(ActualParams ActualParams) { visit(); }
    public void visit(NoActuals NoActuals) { visit(); }
    public void visit(Actuals Actuals) { visit(); }
    public void visit(ForEachLoopParen ForEachLoopParen) { visit(); }
    public void visit(WhileLoopParen WhileLoopParen) { visit(); }
    public void visit(ErrorIfCondition ErrorIfCondition) { visit(); }
    public void visit(IfCondition IfCondition) { visit(); }
    public void visit(PrintEmptyDummy PrintEmptyDummy) { visit(); }
    public void visit(StatementBlock StatementBlock) { visit(); }
    public void visit(StatementForeach StatementForeach) { visit(); }
    public void visit(StatementPrintWithWidth StatementPrintWithWidth) { visit(); }
    public void visit(StatementPrintWithoutWidth StatementPrintWithoutWidth) { visit(); }
    public void visit(StatementRead StatementRead) { visit(); }
    public void visit(StatementReturnExpr StatementReturnExpr) { visit(); }
    public void visit(StatementReturnEmpty StatementReturnEmpty) { visit(); }
    public void visit(StatementContinue StatementContinue) { visit(); }
    public void visit(StatementBreak StatementBreak) { visit(); }
    public void visit(StatementWhile StatementWhile) { visit(); }
    public void visit(StatementIfElse StatementIfElse) { visit(); }
    public void visit(StatementIf StatementIf) { visit(); }
    public void visit(StatementDesignator StatementDesignator) { visit(); }
    public void visit(NoStatements NoStatements) { visit(); }
    public void visit(Statements Statements) { visit(); }
    public void visit(MethodWithoutVariables MethodWithoutVariables) { visit(); }
    public void visit(MethodVariables MethodVariables) { visit(); }
    public void visit(VoidMethodTypeName VoidMethodTypeName) { visit(); }
    public void visit(ReturnMethodTypeName ReturnMethodTypeName) { visit(); }
    public void visit(MethodDeclaration MethodDeclaration) { visit(); }
    public void visit(NoMethodDeclarations NoMethodDeclarations) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(SingleOrEndOfListMethod SingleOrEndOfListMethod) { visit(); }
    public void visit(ListOfMethodsInClass ListOfMethodsInClass) { visit(); }
    public void visit(ConstructorWithoutVar ConstructorWithoutVar) { visit(); }
    public void visit(ConstructorListOfVar ConstructorListOfVar) { visit(); }
    public void visit(SingleFormalParameter SingleFormalParameter) { visit(); }
    public void visit(ErrorSingleOrEndOfListParameter ErrorSingleOrEndOfListParameter) { visit(); }
    public void visit(ErrorListOfParameters ErrorListOfParameters) { visit(); }
    public void visit(SingleOrEndOfListParameter SingleOrEndOfListParameter) { visit(); }
    public void visit(ListOfParameters ListOfParameters) { visit(); }
    public void visit(NoFormalParameters NoFormalParameters) { visit(); }
    public void visit(FormalParameters FormalParameters) { visit(); }
    public void visit(ConstructorDeclarationName ConstructorDeclarationName) { visit(); }
    public void visit(ConstructorDeclaration ConstructorDeclaration) { visit(); }
    public void visit(SingleOrLastConstructorDeclaration SingleOrLastConstructorDeclaration) { visit(); }
    public void visit(ListOfConstructorDeclarations ListOfConstructorDeclarations) { visit(); }
    public void visit(ClassVariable ClassVariable) { visit(); }
    public void visit(ClassVarDecLast ClassVarDecLast) { visit(); }
    public void visit(ClassVarDecNotLast ClassVarDecNotLast) { visit(); }
    public void visit(ErrorClassVarDeclaration ErrorClassVarDeclaration) { visit(); }
    public void visit(ClassVarDeclaration ClassVarDeclaration) { visit(); }
    public void visit(NoClassVariables NoClassVariables) { visit(); }
    public void visit(ClassVariables ClassVariables) { visit(); }
    public void visit(ClassBodyNoMethodWithEmptyBrackets ClassBodyNoMethodWithEmptyBrackets) { visit(); }
    public void visit(ClassBodyWithMethodAndConstructor ClassBodyWithMethodAndConstructor) { visit(); }
    public void visit(ClassBodyWithMethodNoConstructor ClassBodyWithMethodNoConstructor) { visit(); }
    public void visit(ClassBodyNoMethodWithConstructor ClassBodyNoMethodWithConstructor) { visit(); }
    public void visit(ClassBodyNoMethod ClassBodyNoMethod) { visit(); }
    public void visit(ErrorClassExtended ErrorClassExtended) { visit(); }
    public void visit(ClassIsNotExtended ClassIsNotExtended) { visit(); }
    public void visit(ClassIsExtended ClassIsExtended) { visit(); }
    public void visit(NameOfClass NameOfClass) { visit(); }
    public void visit(ClassDeclaration ClassDeclaration) { visit(); }
    public void visit(EndOfListOfCharDeclarations EndOfListOfCharDeclarations) { visit(); }
    public void visit(ListOfCharDeclarations ListOfCharDeclarations) { visit(); }
    public void visit(ConstantValueChar ConstantValueChar) { visit(); }
    public void visit(ConstantValueNumber ConstantValueNumber) { visit(); }
    public void visit(ConstantValueBoolean ConstantValueBoolean) { visit(); }
    public void visit(ConstDeclaration ConstDeclaration) { visit(); }
    public void visit(ErrorVarDeclLast ErrorVarDeclLast) { visit(); }
    public void visit(LastVarDeclarations LastVarDeclarations) { visit(); }
    public void visit(VarDeclarationOfNotArray VarDeclarationOfNotArray) { visit(); }
    public void visit(VarDeclarationOfArray VarDeclarationOfArray) { visit(); }
    public void visit(ErrorVarDeclNotLast ErrorVarDeclNotLast) { visit(); }
    public void visit(NotLastVarDeclarations NotLastVarDeclarations) { visit(); }
    public void visit(VarDeclarationLast VarDeclarationLast) { visit(); }
    public void visit(VarDeclarationNotLast VarDeclarationNotLast) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(VarDeclaration VarDeclaration) { visit(); }
    public void visit(NoVarDecl NoVarDecl) { visit(); }
    public void visit(ClassDeclarations ClassDeclarations) { visit(); }
    public void visit(ConstDeclarations ConstDeclarations) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
